...
async def fetch_anime_news():
    ...
    for entry in feed.entries:
        ...
        # Filter by keywords (e.g., "anime" or "manga")
        if "anime" not in entry.title.lower() and "manga" not in entry.title.lower():
            continue  # Skip non-anime/manga news
...
async def cmd_news(message: types.Message, bot: Bot):
    ...
    if not news_items:
        await message.reply("No anime or manga news available right now.")
        return
